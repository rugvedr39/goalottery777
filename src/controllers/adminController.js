...
const rechargeDuyet = async(req, res) => {
    ...
    if (type == 'confirm') {
        ...
        const [rechcount] = await connection.query(`SELECT COUNT(*) as rech_count FROM recharge WHERE phone = ? AND  status = 1  `, [info[0].phone]);
        ...
        if(rechcount[0].rech_count=='1'){
           let bonus=0.02* info[0].money;
            await connection.query('UPDATE users SET money = money + ?, bonus = bonus + ? WHERE code = ? ', [bonus,bonus,userinfo[0].invite]);
        }
        return res.status(200).json({
...
const settingBank = async(req, res) => {
    ...
    if (typer == 'bank') {
        await connection.query(`UPDATE bank_recharge SET name_bank = ?, name_user = ?, stk = ?, upi = ? WHERE type = 'bank'`, [name_bank, name, info,upi]);
        return res.status(200).json({
    ...
    if (typer == 'momo') {
        await connection.query(`UPDATE bank_recharge SET name_bank = ?, name_user = ?, stk = ?, upi = ? WHERE type = 'momo'`, [name_bank, name, info,upi]);
        return res.status(200).json({
            message: 'Successful change',
            status: true,
            datas: recharge,
