...
const promotion = async(req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `code`,`invite`,`bonus`, `roses_f`, `roses_f1`, `roses_today` FROM users WHERE `token` = ? ', [auth]);
...
const withdrawal3 = async (req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    if (!auth || !money || !password || money <= 319) {
        return res.status(200).json({
            message: 'Minimum Withdraw 310',
            status: false,
            timeStamp: timeNow,
        })
    }
        ...
        if (withdraw.length < 2) {
            if (userInfo.money - money >= 0) {
                if (true) {
                    ...
                    return res.status(200).json({
                        message: 'The First Recharge Amount Must Be "0" To Withdraw Your Winning Amount',
                        status: false,
                        timeStamp: timeNow,
                    });
                }
